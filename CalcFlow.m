...
u_snap = zeros(nx+2, ny+2, loop);
v_snap = zeros(nx+2, ny+2, loop);
p_snap = zeros(nx+2, ny+2, loop);
pf_snap = zeros(nx+2, ny+2, loop);
...
%モード分析用スナップショットデータの追加
save('shapshot.mat', 'nx', 'ny', 'dt', 'loop', 'u_snap', 'v_snap', 'p_snap');
